...
# Sam Schneidereith 2026
# Written Based on the SHELL platform layout
...
class StewartPlatform:
    ...
    def jointLocations(self, rad_base, rad_platform):
        ...
        jointSpacing_base = 2 * 0.0254  # Joint pair spacing
        jointSpacing_platform = 2.75 * 0.0254

        alpha_base = math.asin((jointSpacing_base / 2) / rad_base) if rad_base != 0 else 0
        alpha_platform = math.asin((jointSpacing_platform / 2) / rad_platform) if rad_platform != 0 else 0

        # --- Base joints ---
        B[:, 0] = [0, rad_base * np.cos((1*np.pi/6) + alpha_base), rad_base * np.sin((1*np.pi/6) + alpha_base)]
        B[:, 1] = [0, rad_base * np.cos((5*np.pi/6) - alpha_base), rad_base * np.sin((5*np.pi/6) - alpha_base)]
        B[:, 2] = [0, rad_base * np.cos((5*np.pi/6) + alpha_base), rad_base * np.sin((5*np.pi/6) + alpha_base)]
        B[:, 3] = [0, rad_base * np.cos((9*np.pi/6) - alpha_base), rad_base * np.sin((9*np.pi/6) - alpha_base)]
        B[:, 4] = [0, rad_base * np.cos((9*np.pi/6) + alpha_base), rad_base * np.sin((9*np.pi/6) + alpha_base)]
        B[:, 5] = [0, rad_base * np.cos((1*np.pi/6) - alpha_base), rad_base * np.sin((1*np.pi/6) - alpha_base)]

        # --- Platform joints ---
        P[:, 0] = [0, rad_platform * np.cos((3*np.pi/6)  - alpha_platform), rad_platform * np.sin((3*np.pi/6)  - alpha_platform)]
        P[:, 1] = [0, rad_platform * np.cos((3*np.pi/6)  + alpha_platform), rad_platform * np.sin((3*np.pi/6)  + alpha_platform)]
        P[:, 2] = [0, rad_platform * np.cos((7*np.pi/6)  - alpha_platform), rad_platform * np.sin((7*np.pi/6)  - alpha_platform)]
        P[:, 3] = [0, rad_platform * np.cos((7*np.pi/6)  + alpha_platform), rad_platform * np.sin((7*np.pi/6)  + alpha_platform)]
        P[:, 4] = [0, rad_platform * np.cos((11*np.pi/6) - alpha_platform), rad_platform * np.sin((11*np.pi/6) - alpha_platform)]
        P[:, 5] = [0, rad_platform * np.cos((11*np.pi/6) + alpha_platform), rad_platform * np.sin((11*np.pi/6) + alpha_platform)]
        return B, P
    

if __name__ == '__main__':
    X = np.array([0.5,0,0,0,np.pi/6,0])
